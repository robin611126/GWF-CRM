...
export default function InvoicesPage() {
    ...
    const generateInvoicePDF = async (id) => {
        ...
        let y = 20;
        doc.setFontSize(22); doc.setFont('helvetica', 'bold'); doc.text('INVOICE', 14, y);
        doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(100); doc.text(inv.invoice_number, 14, y + 8); doc.setTextColor(0);
        const statusLabel = STATUS_LABELS[inv.status] || inv.status;
        doc.setFontSize(12); doc.setFont('helvetica', 'bold'); doc.text(statusLabel, pageW - 14, y, { align: 'right' });
        y += 20;
        doc.setFontSize(10); doc.setFont('helvetica', 'normal');
        doc.text('Bill To:', 14, y); doc.setFont('helvetica', 'bold'); doc.text(inv.client?.name || '—', 14, y + 6);
        ...
        y += 30;
        doc.setFillColor(240, 240, 240); doc.rect(14, y, pageW - 28, 8, 'F');
        doc.setFontSize(9); doc.setFont('helvetica', 'bold');
        doc.text('Description', 16, y + 6); doc.text('Qty', 120, y + 6, { align: 'right' }); doc.text('Price', 150, y + 6, { align: 'right' }); doc.text('Total', pageW - 16, y + 6, { align: 'right' });
        y += 12; doc.setFont('helvetica', 'normal');
        (inv.items || []).forEach(item => {
            doc.text(String(item.description || ''), 16, y); doc.text(String(item.quantity), 120, y, { align: 'right' });
            doc.text(`₹${Number(item.unit_price).toLocaleString('en-IN')}`, 150, y, { align: 'right' });
            doc.text(`₹${(Number(item.quantity) * Number(item.unit_price)).toLocaleString('en-IN')}`, pageW - 16, y, { align: 'right' }); y += 7;
        });
        y += 5; doc.setDrawColor(200); doc.line(14, y, pageW - 14, y); y += 8;
        doc.setFont('helvetica', 'bold'); doc.setFontSize(12);
        doc.text('Total:', 130, y); doc.text(`₹${Number(inv.total_amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, pageW - 16, y, { align: 'right' }); y += 8;
        doc.setFontSize(10); doc.setFont('helvetica', 'normal');
        doc.text('Paid:', 130, y); doc.text(`₹${Number(inv.amount_paid).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, pageW - 16, y, { align: 'right' }); y += 7;
        const bal = Number(inv.total_amount) - Number(inv.amount_paid);
        if (bal > 0) { doc.setFont('helvetica', 'bold'); doc.setTextColor(239, 68, 68); doc.text('Balance Due:', 130, y); doc.text(`₹${bal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, pageW - 16, y, { align: 'right' }); doc.setTextColor(0); }
        doc.setFontSize(8); doc.setTextColor(150); doc.text('Generated by GWF CRM', pageW / 2, 285, { align: 'center' });
        return { doc, inv };
    };
    ...
    const handleDownloadPDF = async (id, invoiceNumber) => {
        ...
        } catch (err) { console.error(err); showToast('Failed to download invoice', 'error'); }
    };
    ...
    const handlePreviewPDF = async (id) => {
        ...
        } catch (err) { console.error(err); showToast('Failed to preview invoice', 'error'); }
    };
